...
def gd_step_with_l2(model, lr, weight_decay):
...
for lam in L2_LAMBDAS:
    ...
    cls_base = os.path.join(lam_dir, "classification")
    reg_base = os.path.join(lam_dir, "regression")
    os.makedirs(cls_base, exist_ok=True)
    os.makedirs(reg_base, exist_ok=True)

  
    # CLASSIFICATION (GD + L2)
    print(f"\nCLASSIFICATION (GD + L2 λ = {lam})")
    ...
    for depth in range(1, 5):
        ...
        print(f"[Classification] λ={lam} Depth={depth} | "
        ...
        np.save(os.path.join(cls_base, f"train_acc_depth_{depth}.npy"), epoch_tr_acc)
        np.save(os.path.join(cls_base, f"val_acc_depth_{depth}.npy"), epoch_va_acc)
        np.save(os.path.join(cls_base, f"train_loss_depth_{depth}.npy"), epoch_tr_loss)
        np.save(os.path.join(cls_base, f"val_loss_depth_{depth}.npy"), epoch_va_loss)
        ...
        plt.xlabel("Epochs")
        plt.ylabel("Accuracy")
        plt.title(f"GD+L2 (λ={lam}) Classification Accuracy (Depth={depth})")
        plt.legend()
        plt.savefig(os.path.join(cls_base, f"acc_depth_{depth}.png"))
        plt.close()
        ...
        plt.xlabel("Epochs")
        plt.ylabel("Loss")
        plt.title(f"GD+L2 (λ={lam}) Classification Loss (Depth={depth})")
        plt.legend()
        plt.savefig(os.path.join(cls_base, f"loss_depth_{depth}.png"))
        plt.close()
    ...
    cls_df.to_csv(os.path.join(cls_base, "classification_results.csv"), index=False)



    # REGRESSION (GD + L2)
    print(f"\nREGRESSION (GD + L2 λ = {lam})")
    ...
    for depth in range(1, 5):
        ...
        print(f"[Regression] λ={lam} Depth={depth} | Train MSE={epoch_tr_mse[-1]:.4f} | "
              f"Val MSE={epoch_va_mse[-1]:.4f} | R2={r2:.4f}")

        reg_results.append([depth, float(epoch_tr_mse[-1]), float(epoch_va_mse[-1]), float(r2)])

        np.save(os.path.join(reg_base, f"train_mse_depth_{depth}.npy"), epoch_tr_mse)
        np.save(os.path.join(reg_base, f"val_mse_depth_{depth}.npy"), epoch_va_mse)
        ...
        plt.xlabel("Epochs")
        plt.ylabel("MSE")
        plt.title(f"GD+L2 (λ={lam}) Regression MSE (Depth={depth})")
        plt.legend()
        plt.savefig(os.path.join(reg_base, f"mse_depth_{depth}.png"))
        plt.close()
        ...
        plt.xlabel("Epochs")
        plt.ylabel("Loss")
        plt.title(f"GD+L2 (λ={lam}) Regression Loss (Depth={depth})")
        plt.legend()
        plt.savefig(os.path.join(reg_base, f"loss_depth_{depth}.png"))
        plt.close()
    ...
    reg_df.to_csv(os.path.join(reg_base, "regression_results.csv"), index=False)

    print(f"\nAll GD+L2 experiments for λ={lam} finished. Results saved in: {lam_dir}")

print("\nAll GD+L2 runs finished for all λ values.")