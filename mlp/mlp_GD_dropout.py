...
KFOLDS = 5
LR = 0.001
DROPOUT_RATE = 0.3   # 🔥 Dropout strength

BASE_DIR = "results/gd_dropout"
...
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim, depth):
        ...
        for _ in range(depth):
            ...
            layers.append(nn.Dropout(DROPOUT_RATE))  # 🔥 Dropout added
            prev = HIDDEN_NEURONS
...
for depth in range(1, 5):
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X_train)):
        ...
        for e in range(EPOCHS):
            ...
            with torch.no_grad():
                vo = model(Xv)
                epoch_va_mse[e] += loss_fn(vo, yv).item()
    ...
    print(f"[Regression] Depth={depth} | "
          f"Train MSE={epoch_tr_mse[-1]:.4f} | "
          f"Val MSE={epoch_va_mse[-1]:.4f} | "
          f"R2={r2:.4f}")
...
print("\nAll GD + Dropout experiments finished. Results saved in:", BASE_DIR)